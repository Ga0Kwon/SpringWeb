...
export default function Chatting(props) { //export : 내보내기
   ...
   webSocket.current.onopen = ()=>{console.log("웹 소켓 접속했습니다.");}
   ...
   webSocket.current.onmessage =  (e)=>{
        ...
        setMsgContent([...msgContent, e.data]); //재 렌더링, 기존 배열에 e.data를 넣어주는
   }
...
/*        console.log(msgInput.current.value);
        ...
        webSocket.current.send(msg); //클라이언트가 서버에게 메시지를 전송 [.send()]
